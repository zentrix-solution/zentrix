      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <Card 
              key={index} 
              className="p-6 rounded-2xl border-0 shadow-lg hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 cursor-pointer group"
            >
        ...
        <div className="flex justify-center items-center mt-16 space-x-8">
          {[1, 2, 3, 4, 5].map((star) => (
            <div key={star} className="text-4xl text-yellow-400 transform hover:scale-125 transition-transform duration-200">
